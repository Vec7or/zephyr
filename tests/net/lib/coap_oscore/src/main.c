...
ZTEST(coap_oscore, test_oscore_exchange_cache_eviction)
{
	...
	r = oscore_exchange_add(cache, (struct net_sockaddr *)&new_addr, sizeof(new_addr), token,
				sizeof(token));
	zassert_equal(r, -ENOMEM, "Should prevent adding new entry when cache is full");

	/* Verify new entry does not exists */
	struct coap_oscore_exchange *entry;
...
ZTEST(coap_oscore, test_oscore_e2e_response_encrypted)
{
	...
	zassert_equal(ret, 0, "Client context init failed (%d)", ret);
	...
	zassert_equal(ret, 0, "Server context init failed (%d)", ret);

	ret = coap_service_start(&oscore_secure_service);
	zassert_equal(ret, 0, "Failed to start service (%d)", ret);
	...
	zassert_equal(ret, 0, "Failed to build request (%d)", ret);

	ret = oscore_send_protected(sock, client_ctx, &dst, &req);
	zassert_equal(ret, 0, "Failed to send request (%d)", ret);
	...
	zassert_equal(zsock_close(sock), 0, "Failed to close socket");
	zassert_equal(coap_service_stop(&oscore_secure_service), 0, "Failed to stop service");
...
ZTEST(coap_oscore, test_oscore_required_rejects_plaintext)
{
	...
	zassert_equal(ret, 0, "Failed to parse response (%d)", ret);
...
ZTEST(coap_oscore, test_oscore_mixed_sync_and_plaintext)
{
	...
	zassert_equal(ret, 0, "Failed to start mixed service (%d)", ret);
	...
	zassert_equal(ret, 0, "Failed to build OSCORE request (%d)", ret);

	ret = oscore_send_protected(sock, client_ctx, &dst, &req);
	zassert_equal(ret, 0, "Failed to send OSCORE request (%d)", ret);
	...
	zassert_equal(ret, 0, "Failed to build plaintext request (%d)", ret);

	ret = oscore_send_plain(sock, &dst, &req);
	zassert_equal(ret, 0, "Failed to send plaintext request (%d)", ret);

	ret = oscore_recv(sock, LONG_TIMEOUT, recvbuf, sizeof(recvbuf));
	zassert_true(ret > 0, "Timed out waiting for plaintext response (%d)", ret);

	ret = coap_packet_parse(&rsp, recvbuf, ret, NULL, 0);
	zassert_equal(ret, 0, "Failed to parse plaintext response (%d)", ret);
	...
	zassert_equal(zsock_close(sock), 0, "Failed to close socket");
	zassert_equal(coap_service_stop(&oscore_mixed_service), 0, "Failed to stop mixed service");
	coap_oscore_context_free(client_ctx);
...
ZTEST(coap_oscore, test_oscore_observe_notifications_encrypted)
{
	...
	zassert_equal(ret, 0, "Failed to build observe request (%d)", ret);
	ret = oscore_send_protected(sock, client_ctx, &dst, &req);
	zassert_equal(ret, 0, "Failed to send observe request (%d)", ret);

	/* Initial notification must be OSCORE-protected. */
	inner_len = sizeof(innerbuf);
	ret = oscore_recv_verify(sock, LONG_TIMEOUT, client_ctx, innerbuf, &inner_len, &inner);
	zassert_equal(ret, 0, "Initial notification verify/parse failed (%d)", ret);
	...
	zassert_equal(ret, 0, "coap_resource_notify failed (%d)", ret);

	inner_len = sizeof(innerbuf);
	ret = oscore_recv_verify(sock, LONG_TIMEOUT, client_ctx, innerbuf, &inner_len, &inner);
	zassert_equal(ret, 0, "Async notification verify/parse failed (%d)", ret);
	zassert_true(coap_get_option_int(&inner, COAP_OPTION_OBSERVE) >= 0,
	...
	zassert_equal(ret, 0, "Failed to build deregister request (%d)", ret);
	ret = oscore_send_protected(sock, client_ctx, &dst, &req);
	zassert_equal(ret, 0, "Failed to send deregister request (%d)", ret);

	inner_len = sizeof(innerbuf);
	ret = oscore_recv_verify(sock, LONG_TIMEOUT, client_ctx, innerbuf, &inner_len, &inner);
	zassert_equal(ret, 0, "Deregister response verify/parse failed (%d)", ret);

	/* No further notifications once the observation is gone. */
	ret = coap_resource_notify(&oscore_observe_resource);
	zassert_equal(ret, 0, "coap_resource_notify after deregister failed (%d)", ret);
...
ZTEST(coap_oscore, test_oscore_observe_plaintext_not_encrypted)
{
	...
	ret = coap_packet_parse(&rsp, recvbuf, ret, NULL, 0);
	zassert_equal(ret, 0, "Failed to parse initial notification (%d)", ret);
	zassert_false(coap_oscore_msg_has_oscore(&rsp),
	...
	ret = coap_packet_parse(&rsp, recvbuf, ret, NULL, 0);
	zassert_equal(ret, 0, "Failed to parse async notification (%d)", ret);
	zassert_false(coap_oscore_msg_has_oscore(&rsp),
	...
	ret = coap_packet_parse(&rsp, recvbuf, ret, NULL, 0);
	zassert_equal(ret, 0, "Failed to parse deregister response (%d)", ret);
...
ZTEST(coap_oscore, test_oscore_empty_ack_and_deferred)
{
	...
	zassert_equal(ret, 0, "Failed to parse empty ACK (%d)", ret);
	...
	entry = oscore_exchange_find(oscore_secure_service.data->oscore_exchange_cache,
	...
	zassert_equal(ret, 0, "Failed to send deferred response (%d)", ret);

	ret = oscore_recv_verify(sock, LONG_TIMEOUT, client_ctx, innerbuf, &inner_len, &inner);
	zassert_equal(ret, 0, "Deferred response verify/parse failed (%d)", ret);
	zassert_equal(coap_header_get_code(&inner), COAP_RESPONSE_CODE_CONTENT,
...
ZTEST(coap_oscore, test_oscore_deferred_after_expiry_mixed_plaintext)
{
	...
	entry = oscore_exchange_find(oscore_mixed_service.data->oscore_exchange_cache,
				     net_sad(&oscore_deferred.addr), oscore_deferred.addr_len,
				     oscore_deferred.token, oscore_deferred.tkl);
	zassert_not_null(entry, "Exchange must exist before expiry");
	entry->timestamp = k_uptime_get() - CONFIG_COAP_OSCORE_EXCHANGE_LIFETIME_MS - 1000;
	...
	zassert_equal(ret, 0, "Mixed deferred response should still be sent (%d)", ret);

	ret = oscore_recv(sock, LONG_TIMEOUT, recvbuf, sizeof(recvbuf));
	zassert_true(ret > 0, "Timed out waiting for deferred response (%d)", ret);
	ret = coap_packet_parse(&rsp, recvbuf, ret, NULL, 0);
	zassert_equal(ret, 0, "Failed to parse deferred response (%d)", ret);
	zassert_false(coap_oscore_msg_has_oscore(&rsp),
...
ZTEST(coap_oscore, test_oscore_empty_ack_does_not_clear_exchange)
{
	...
	ret = oscore_exchange_add(cache, (struct net_sockaddr *)&addr, sizeof(addr), token,
				  sizeof(token));
	zassert_equal(ret, 0, "Should add exchange entry");

	/* Empty ACK removal uses a zero-length token; it must not match. */
	oscore_exchange_remove(cache, (struct net_sockaddr *)&addr, sizeof(addr), token, 0);
...
ZTEST(coap_oscore, test_unsupported_critical_option_helper)
{
	...
	r = coap_packet_init(&cpkt, buffer, sizeof(buffer), COAP_VERSION_1, COAP_TYPE_CON, 0, NULL,
	...
	r = coap_packet_append_option(&cpkt, COAP_OPTION_OSCORE, oscore_value,
				      sizeof(oscore_value));
	...
	r = coap_packet_init(&cpkt, buffer2, sizeof(buffer2), COAP_VERSION_1, COAP_TYPE_CON, 0,
...
ZTEST(coap_oscore, test_server_rejects_unsupported_critical_con_request)
{
	...
	zassert_equal(ret, 0, "Failed to start emulation CoAP service (%d)", ret);
	...
	zassert_equal(coap_service_stop(&coap_service), 0,
