...
static int recv_response_tcp(struct coap_client_tcp *client,
			     struct coap_packet *response);
static int handle_response_tcp(struct coap_client_tcp *client,
...
static int receive_tcp(struct coap_client_tcp *client, int flags)
{
	...
	if (received < 0) {
		if (errno == EAGAIN || errno == EWOULDBLOCK) {
			/* No new data, but check if buffer already has a complete packet */
			if (offset == 0) {
				return -EAGAIN;
			}
			goto parse;
					...
					struct coap_client_tcp_internal_request *internal_req)
{
	int ret = 0;
	bool block2 = false;
	bool block_continuation = internal_req->recv_blk_ctx.current > 0 ||
				  internal_req->send_blk_ctx.current > 0;

	memset(internal_req->send_buf, 0, sizeof(internal_req->send_buf));

	/* RFC 8323 Section 5.3: Signaling messages (class 7) MUST NOT include a token */
	bool is_signaling = ((uint8_t)req->method >> 5) == 7;
	uint8_t tkl;

	if (is_signaling) {
		tkl = 0;
		internal_req->request_tkl = 0;
	} else {
		/* Generate new token unless continuing blockwise with token reuse enabled */
		if (!block_continuation ||
		    !IS_ENABLED(CONFIG_COAP_CLIENT_TCP_BLOCKWISE_REUSE_TOKEN)) {
	...
	if (req->payload || req->payload_cb) {
		...
		if (req->payload_cb) {
			...
			size_t cb_offset = internal_req->send_blk_ctx.current;
		 ...
		 */
		use_blockwise = client->blockwise_enabled &&
...
static int write_csm_option_value_u32(struct coap_client_tcp_option *option,
				      size_t length, uint32_t val)
{
	unsigned int n, i;
	...
	for (int i = 0; i < nfds; i++) {
		...
		if (fds[i].revents & ZSOCK_POLLIN) {
			do {
				...
				/* Consume processed packet, preserve remaining */
				if (client->recv_offset > pkt_len) {
					memmove(client->recv_buf,
						client->recv_buf + pkt_len,
						client->recv_offset - pkt_len);
					client->recv_offset -= pkt_len;
				} else {
					client->recv_offset = 0;
				}
			} while (client->recv_offset > 0);
		}
