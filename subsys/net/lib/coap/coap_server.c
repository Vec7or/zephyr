...
#if defined(CONFIG_COAP_OSCORE)
/* True while coap_server_process() is handling an OSCORE-verified request.
 * Read by coap_service_send_internal() to protect synchronous responses
 * without depending on the (expirable) exchange cache. Safe because the
 * server is single-threaded and fully serialized under `lock`.
 */
static bool server_cur_req_oscore;
...
static inline void *coap_server_alloc(size_t len)
{
	...
	if (len > COAP_SERVER_WIRE_MESSAGE_SIZE) {
		return NULL;
	}
...
static int coap_service_remove_observer(const struct coap_service *service,
					...
					const struct net_sockaddr *addr, const uint8_t *token,
					uint8_t tkl)
{
...
static int coap_server_process(int sock_fd)
{
	...
	if (ret == -ENOTSUP) {
		...
		if (msg_type == COAP_TYPE_CON) {
			...
			ret = zsock_sendto(sock_fd, rst.data, rst.offset, 0, net_sad(&client_addr),
					   client_addr_len);
			if (ret < 0) {
...
#if defined(CONFIG_COAP_OSCORE)
	request_has_oscore = coap_oscore_msg_has_oscore(&request);
#endif

	if (received > sizeof(buf) ||
	    (!request_has_oscore && received > CONFIG_COAP_SERVER_MESSAGE_SIZE)) {
		/* The message was truncated or exceeds the plaintext CoAP limit. */
		struct coap_packet response;
		uint8_t token[COAP_TOKEN_MAX_LEN];
		uint8_t tkl = coap_header_get_token(&request, token);
		uint16_t id = coap_header_get_id(&request);
	...
	if (oscore_ctx != NULL && request_has_oscore) {
		...
		/* Copy the decrypted CoAP message back into buf and re-parse it
		 * from there, so that request (and the rest of this function)
		 * consistently reference buf rather than the scratch buffer.
		 */
		...
		if (request_has_oscore) {
			request.is_oscore =
				true; /* persists into observer via coap_observer_init */
			server_cur_req_oscore = true; /* bridge for synchronous responses */
		}
		...
		if (!is_observe) {
			ret = oscore_exchange_add(service->oscore_exchange_cache,
			...
			if (ret < 0) {
				...
				LOG_ERR("Failed to add OSCORE exchange entry (%d)", ret);
				ret = -EINVAL;
				goto unlock;
			}
		}
...
unlock:
#if defined(CONFIG_COAP_OSCORE)
	/* Clear the OSCORE request flag after processing the request */
	server_cur_req_oscore = false;
#endif
...
int coap_service_start(const struct coap_service *service)
{
	...
	if (service->sec_tag_list != NULL) {
		...
		ret = zsock_setsockopt(service->data->sock_fd, ZSOCK_SOL_TLS,
				       ZSOCK_TLS_SEC_TAG_LIST, service->sec_tag_list,
				       service->sec_tag_list_size);
		if (ret < 0) {
			ret = -errno;
			goto close;
		}
				      ...
				      bool skip_oscore)
{
	 ...
	 */
	static uint8_t oscore_buf[COAP_SERVER_WIRE_MESSAGE_SIZE];
	uint32_t oscore_len = sizeof(oscore_buf);
	struct coap_oscore_exchange *exchange = NULL;
	struct coap_observer *observer = NULL;
	...
	if (oscore_ctx != NULL && !skip_oscore) {
		...
		} else if (server_cur_req_oscore) {
			/* synchronous response to the in-flight OSCORE request: authoritative,
			 * no cache lookup, so it can never age out.
			 */
			protect = true;
		} else {
			/* Look up exchange to see if this response needs OSCORE protection */
			exchange = oscore_exchange_find(service->oscore_exchange_cache, addr,
							addr_len, token, tkl);
		...
		if (protect) {
			...
			if (ret < 0) {
				...
				/* The exchange is finished (this response will not be sent or
				 * retried); drop its cache entry so the slot is not held until the
				 * exchange lifetime expires.
				 */
				if (!is_notify && (exchange != NULL || server_cur_req_oscore)) {
					oscore_exchange_remove(service->oscore_exchange_cache, addr,
							       addr_len, token, tkl);
				}
				(void)k_mutex_unlock(&lock);
				return ret;
			...
			LOG_DBG("OSCORE protected response: %zu -> %zu bytes", cpkt->offset,
				send_len);
		} else if (service->oscore_required && !is_empty) {
			/* Service requires OSCORE but response is not protected */
			LOG_WRN("Service requires OSCORE but response is not protected");
			(void)k_mutex_unlock(&lock);
			return -EACCES;
		}
	}
...
send:
	...
	if (oscore_ctx != NULL && !skip_oscore) {
		...
		if (!is_notify && !is_empty && (exchange != NULL || server_cur_req_oscore)) {
			uint8_t token[COAP_TOKEN_MAX_LEN];
			uint8_t tkl = coap_header_get_token(cpkt, token);

			oscore_exchange_remove(service->oscore_exchange_cache, addr, addr_len,
					       token, tkl);
		}
	}
#endif
...
#if defined(CONFIG_COAP_OSCORE)
		coap_observer_init(observer, request, addr, request->is_oscore);
#else
