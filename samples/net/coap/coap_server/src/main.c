...
COAPS_SERVICE_DEFINE(coap_server, NULL, &coap_port, 0, sec_tag_list_verify_none,
...
static int setup_dtls(void)
{
	...
	err = tls_credential_add(SERVER_CERTIFICATE_TAG, TLS_CREDENTIAL_CA_CERTIFICATE,
	...
	err = tls_credential_add(SERVER_CERTIFICATE_TAG, TLS_CREDENTIAL_PUBLIC_CERTIFICATE,
	...
	err = tls_credential_add(SERVER_CERTIFICATE_TAG, TLS_CREDENTIAL_PRIVATE_KEY, private_key,
				 sizeof(private_key));
	if (err < 0) {
