...
int oscore_exchange_add(struct coap_oscore_exchange *cache, const struct net_sockaddr *addr,
			net_socklen_t addr_len, const uint8_t *token, uint8_t tkl, bool is_observe);
...
enum coap_oscore_aead_alg {
	/** Use the OSCORE default (AES-CCM-16-64-128). */
	COAP_OSCORE_AEAD_DEFAULT = 0,
	/** AES-CCM mode 128-bit key, 64-bit tag, 13-byte nonce. */
	COAP_OSCORE_AEAD_AES_CCM_16_64_128 = 10,
};

/** @brief HKDF algorithm used to derive OSCORE keys (RFC 8613 Section 3.1). */
enum coap_oscore_hkdf {
	/** Use the OSCORE default (HKDF-SHA-256). */
	COAP_OSCORE_HKDF_DEFAULT = 0,
	/** HKDF-SHA-256. */
...
struct coap_oscore_init_params {
	...
	/** AEAD algorithm (use ::COAP_OSCORE_AEAD_DEFAULT for the default). */
	enum coap_oscore_aead_alg aead_alg;
	/** KDF algorithm (use ::COAP_OSCORE_HKDF_DEFAULT for the default). */
	enum coap_oscore_hkdf hkdf;

	/**
	 * True if the combination of Master Secret and Master Salt is unique at
	 * every boot (e.g. derived with EDHOC). If false, the sender sequence
	 * number must be persisted to non-volatile memory to prevent reuse.
	 */
 ...
 *
 * @param[in]  params Derivation parameters.
 * @param[out] ctx    On success, set to the newly created context handle.
 *
 * @retval 0 on success.
